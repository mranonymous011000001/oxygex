...
pub fn core_main() -> Option<Vec<String>> {
    ...
    if args.is_empty() || crate::common::is_empty_uni_link(&args[0]) {
        ...
        if args[0] == "--remove" {
            ...
            eprintln!("[oxygex-controller] server/service mode disabled in controller-only build");
            return None;
        } else if args[0] == "--server" {
            eprintln!("[oxygex-controller] server mode disabled in controller-only build");
            return None;
